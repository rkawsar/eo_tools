...
rr = raw_input("Give this run a number") #This run number eliminates the need to go back and delete files if executing this script multiple times
r = str(rr)
print "This is now run " + r
raw_input("Press enter to continue")
#Reset these parameters accordingly. Remember to use \\ instead of \ in file paths
d = "I:\\accuracy_assessment\\" #base directory, needs an "outputs" folder and a "scratch" folder and a "final_merge" folder
training_poly = "I:\\training_sites.shp" #training polygons shapefile
POINTS = "50" #number of points
landcover_in = "I:\\post_process_r7\\r7_final" #input land cover map
rand_space = "90" #Minimum distance between randomized points in meters. Can be 0 if desired
lc_count = 10 #Highest land cover class grid code. Land cover raster must contain all sequential values between 1 and lc_count.
...
assessment_squares_run_shp = d + "final_merge\\assessment_squares_run" + r + ".shp" #this is the final output of the stratified random sampling
# Process: Focal Statistics- targets center pixels of homogenous land cover arcpy.gp.FocalStatistics_sa(landcover_in, focal_stats_run_img, "Rectangle 3 3 CELL", "VARIETY", "DATA") print "focal stats finished"
# Process: Reclassify- eliminates all pixels not surrounded by homogenous lc arcpy.gp.Reclassify_sa(focal_stats_run_img, "VALUE", "1 1", reclass_run_img, "NODATA")
...
￼
x = str(intx)
class_run_all_shp = d + "scratch\\class" + x + "_run" + r + "_all.shp" arcpy.Select_analysis(value_to_point_run_shp, class_run_all_shp, "\"RASTERVALU\" = " + x) #grabs points of
